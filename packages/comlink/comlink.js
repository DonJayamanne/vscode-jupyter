(function (global, factory) {
    ...
    function expose(obj) {
        ...
        const ep = parentPort
        parentPort.on("message", function callback(ev) {
            // if (!ev || !ev) {
            //     return;
            // }
            ...
            //     return;
            // }
            const { id, type, path } = Object.assign({ path: [] }, ev);
            const argumentList = (ev.argumentList || []).map(fromWireValue);
            let returnValue;
            try {
                ...
                switch (type) {
                    ...
                    case "SET" /* MessageType.SET */:
                        {
                            parent[path.slice(-1)[0]] = fromWireValue(ev.value);
                            returnValue = true;
                        }
                        break;
