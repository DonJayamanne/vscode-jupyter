...
export class RemoteKernelForKernelSpec implements IKernel {
    ...
    constructor(
        ...
        public readonly metadata: Readonly<KernelSpecConnectionMetadata>,
    ...
    private async startNewKernel() {
        const server = RemoteJupyterAuthProvider.getServerByFileScheme(this.uri.scheme);
        if (!server) {
        ...
        const sessionManager = new SessionManager({ serverSettings: server.info });
        this._session = await sessionManager.startNew({
            path: this.uri.fsPath.substring(1),
            kernelName: this.metadata.kernelSpec.name
        });
        this.kernelExecution.notebook = {
            session: this._session.kernel,
            getLoggers: () => []
