...
import type { IKernel, IKernels, IKernelSpecs } from './tokens';
...
export class Kernels implements IKernels {
    ...
    async startNew(options: Kernels.IKernelOptions): Promise<Kernel.IModel> {
        ...
        const hook = (kernelId: string, clientId: string, socket: WebSocketClient): void => {
            ...
            socket.on('message', async (message: string | ArrayBuffer | Blob | ArrayBufferView) => {
                let msg;
                ...
                if (msg.header.msg_type === 'input_reply') {
                    kernel.handleMessage(msg).catch((ex) => console.error(`Error in handling kernel message`, ex));
                } else {
                    void processMsg(msg).catch((ex) => console.error(`Error in processing kernel message`, ex));
                }
            });
        ...
        const kernelUrl = `ws://DUMMY:8015/api/kernels/${kernelId}/channels`;
        ...
        wsServer.on('close', (): void => {
            Array.from(this._clients.keys()).forEach((clientId) => {
                const socket = this._clients.get(clientId);
    ...
    private _kernels = new Map<string, IKernel>();
    private _clients = new Map<string, WebSocketClient>();
    private _kernelClients = new Map<string, Set<string>>();
